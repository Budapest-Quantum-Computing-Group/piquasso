...
class TestProgramRegistry(TestProgramBase):
    ...
    def test_single_mode_multiple_operation_registry(self, DummyOperation):
        ...
        assert self.operations[1].params == (42, 320)
    ...
    def test_multiple_mode_single_operation_registry(self, DummyOperation):
        ...
        assert len(self.operations) == 3

        assert self.operations[0].modes == (2, 1, 0)
        assert self.operations[0].params == (421,)

        assert self.operations[1].modes == (1,)
        assert self.operations[1].params == (1,)

        assert self.operations[2].modes == (0, 2)
        assert self.operations[2].params == (999,)
    ...
    def test_multiple_mode_multiple_operation_registry(self, DummyOperation):
        ...
        assert len(self.operations) == 5

        assert self.operations[0].modes == (4,)
        assert self.operations[0].params == (2,)

        assert self.operations[1].modes == (4,)
        assert self.operations[1].params == (0,)

        assert self.operations[2].modes == (0, 2)
        assert self.operations[2].params == (999,)

        assert self.operations[3].modes == (1, 0)
        assert self.operations[3].params == (1,)

        assert self.operations[4].modes == (1, 0)
        assert self.operations[4].params == (9,)
...
class TestModelessOperationRegistry(TestProgramBase):
    ...
    def test_single_modeless_operation_registry(self, DummyModelessOperation):
        ...
        assert len(self.operations) == 1
        assert self.operations[0].params == (42,)
        assert self.operations[0].modes is ModelessMode
    ...
    def test_multiple_modeless_operation_registry(self, DummyModelessOperation):
        ...
        assert len(self.operations) == 3
        assert self.operations[0].params == (4,)
    ...
    def test_mixed_operation_registry(self, DummyOperation, DummyModelessOperation):
        ...
        assert self.operations[0].params == (0,)
        assert self.operations[0].modes == (0, 1)

        assert self.operations[1].params == (10,)
        assert self.operations[1].modes is ModelessMode

        assert self.operations[2].params == (2,)
        assert self.operations[2].modes == (3,)

        assert self.operations[3].params == (1,)
        assert self.operations[3].modes == (3,)

        assert self.operations[4].params == (9,)
        assert self.operations[4].modes is ModelessMode
