...
def _beamsplitter5050_coeff(n, m, N):
    ...
    return (
        2 ** (-(n + m) / 2)
        * (-1) ** (m - N)
        * np.sqrt(factorial(N) * factorial(M) / (factorial(n) * factorial(m)))
...
@lru_cache(maxsize=None)
def calculate_beamsplitter5050_on_fock_space(cutoff):
    return [
...
def _apply_beamsplitter5050(state, modes):
    ...
    subspace_transformations = calculate_beamsplitter5050_on_fock_space(cutoff)
