...
class Expression:
    ...
    def __call__(self, x: Tuple) -> Any:
        # coerce to tuple to avoid surprises with lists
        return self._eval(self._tree.body, tuple(x))
            ...
            if isinstance(node.op, ast.Or):
                for v in node.values:
                    if self._eval(v, x):
                        return True
                return False
            raise InvalidExpression(f"Unsupported bool op: {type(node.op).__name__}")
