...
@lru_cache()
def get_partitions(boxes: int, particles: int) -> List[npt.NDArray[np.intc]]:
...
@lru_cache()
def get_X(d: int) -> npt.NDArray[np.complex128]:
...
def _hafnian(
    A: npt.NDArray,
    polynom_function: Callable[[npt.NDArray, List[int], int], List[float]]
) -> float:
    """
...
def _get_polynom_coefficients(
    A: npt.NDArray, indices: List[int], degree: int
) -> List[float]:
