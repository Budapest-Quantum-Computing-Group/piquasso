...
pytestmark = pytest.mark.benchmark(
    group="application_dense_subgraph_search_with_5_nodes",
)

def piquasso_benchmark(benchmark, adj_matrix_5):
    @benchmark
    def func():
        pq.constants.seed(40)
        adjacency_matrix = adj_matrix_5
        ...
        result.dense_search(subgraphs, TA_graph, 2, 3, max_count=3)


def strawberryfields_benchmark(benchmark, adj_matrix_5):
    @benchmark
    def func():
        adjacency_matrix = adj_matrix_5
        TA_graph = nx.Graph(adjacency_matrix)
        samples = sample.sample(adjacency_matrix, 5, 10)
        subgraphs = sample.to_subgraphs(samples, TA_graph)
        subgraph.search(subgraphs, TA_graph, 2, 3, max_count=3)
